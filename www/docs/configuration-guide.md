...
- **Serveur de configuration local** : pour que les changements soient
  persistés dans le fichier `config/config.ini`, un serveur Node/Express doit
  être en cours d'exécution (`npm start` ou via Electron). Sans serveur,
  seules les modifications dans `localStorage` sont enregistrées et vous
  serez invité à télécharger un fichier `config.ini` via la fonction
  d'export (`ConfigService.exportIni()`).
- Sur mobile (Capacitor), aucune persistance sur fichier n'est disponible.
  Le stockage se limite à `localStorage`/SQLite ou à un service externe.

## 📁 Structure minimum des fichiers liés à la configuration

```
config/
├── config.ini      # fichier INI principal
scripts/
├── configManager.js   # gestion de la modale (UI)
src/
├── services/
│   └── ConfigService.js  # logique de persistance et fallback
```
