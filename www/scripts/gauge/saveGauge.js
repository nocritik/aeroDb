...
// saving a gauge now also pushes the configuration to config.ini via the
// backend API. the function returns a promise that resolves when the
// server call has finished so callers can wait before reloading the page.
function saveNewGauge() {
    ...
    // également enregistrement côté serveur
    const gaugeObject = {
        id: newId,
        instrument,
        gradValue: tabGrad,
    ...
    return ConfigService.saveToIni(gaugeObject);
};
